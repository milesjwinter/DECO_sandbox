...
if __name__ == '__main__':
    ...
    with warnings.catch_warnings():
        ...
        if prev_labels.shape == (0,):
            existing_labels = []
        elif prev_labels.shape ==(4,):
            existing_labels = prev_labels[3]
        else:
            existing_labels = prev_labels[:,3]
        del prev_labels
    ...
    #for filename in glob.iglob('%s/*.png' % indir):
        ...
        if filename not in existing_labels:
            ...
            try:
                ...
                f.write("{}\t{}\t{}\t{}".format(-1, -1, "noise", event_id))
                f.write("\n")
                break
            #img = Image.open(filename).convert("L")
		...
		if len(groups) == 0:
		    f.write("{}\t{}\t{}\t{}".format(-1, -1, "empty", event_id))
		    f.write("\n")
                else:
			...
			if blob_type == 'exit':
			    ...
			    plt.close()
			    f.write("{}\t{}\t{}\t{}".format(Xavg, Yavg, blob_type, event_id))
			    f.write("\n")
