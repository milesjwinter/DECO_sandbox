...
#data = np.loadtxt('compared_labels.txt',dtype='S')
...
#f = h5py.File("Binary_RGB_DECO_Image_Database_100_new.h5","w")
#f = h5py.File("Binary_RGB_DECO_Image_Database_100_new.h5","w")
f = h5py.File("DCGAN_DECO_Image_Database_64.h5","w")
#training_images = f.create_dataset("train/train_images", (1,64,64,1), maxshape=(None,64,64,1), dtype="uint8",chunks=True)
...
for i in random_index:
	...
	cropped_img = np.array(img.crop((x0,y0,x1,y1)))
	    ...
	    #gray_img = gray_img.reshape(64,64,1)
            #training_images[-1] = cropped_img
